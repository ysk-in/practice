...
path_user = r'/mnt/c/tmp/ml-100k/u.user'
path_data = r'/mnt/c/tmp/ml-100k/u.data'
path_item = r'/mnt/c/tmp/ml-100k/u.item'


def get_data(file_path, columns, separator, usecols=None, encoding=None):
...
def get_lens_data():
    ...
    users = get_data(path_user, columns, '|')
    columns = ['user_id', 'movie_id', 'rating', 'unix_timestamp']
    ratings = get_data(path_data, columns, '\t')
    columns = ['movie_id', 'title', 'release_date', 'video_release_date', 'imbd_url']
    movies = get_data(path_item, columns, '|', range(5), 'latin1')
    # 各データをmergeする 結合方法(on=, how=)は指定していないため，既定値(共通のキー, 内部結合)でマージされる。
    movie_ratings = pd.merge(movies, ratings)
    lens = pd.merge(movie_ratings, users)
    return lens, users, ratings, movies


def print_movie_lens_data():
    lens, users, ratings, movies = get_lens_data()
    # 各データをカラム名を付けて表示する
    print(ratings.head())
    print(movies.head())
    # 各データをmergeする 結合方法(on=, how=)は指定していないため，既定値(共通のキー, 内部結合)でマージされる。
