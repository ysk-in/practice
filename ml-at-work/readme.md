...
- 嗜好データ：ユーザのアイテムに対する「好き」という感情 や 5段階で「２」という評価など，その好みを表すもの。  
...
`ユーザの情報をベクトルで表現 -> ユーザ間の類似度を決める -> 類似度に基づいて評価値を算出`  

類似度としては代表的なものとして以下がある。値が大きければ似ており，小さければ似ていない。  

- [ピアソンの相関係数](https://blog.apar.jp/data-analysis/2822/)：相関係数と呼ぶときはこれを指すことが多い  
- コサイン類似度：テキストの文章間の距離を算出するときによく利用する  
- ジャッカード係数：集合間の距離を計算することができる  

どの類似度が良いかは問題によって変わるため，色々な方法を試すのが良い。  



#### 7.2.5  アイテム間型協調フィルタリング

ピアソンの相関係数やコサイン類似度などで類似度が高いアイテムを見つけるという解き方をする。  
計算方法はユーザ間型協調フィルタリングと大きく変わらない。  

アイテムに対する類似アイテムを表示するのは，まだアクティビティが少ないユーザに対しても提示できるため，登録してすぐのユーザにもアプローチしやすい方法。反面，人気のアイテムばかりが推薦されるという課題も顕著なため，意外性を出すには工夫が必要。  



#### 7.2.6  モデルベース協調フィルタリング

教師あり学習 や 教師なし学習のモデルをつくることで，既知のデータの規則性を元に予測する方法。  
モデルには以下を使った方法がある。  

- クラスタリング：嗜好が似ているユーザのグループを作成し，所属するグループに好まれるものを推薦する方法。  
- 回帰：線形回帰などの回帰モデルを学習して評価値を予測する。  
- トピックモデル：PLSA(Probabilistic Latent Semantic analysis)やLDA(Latent Dirichlet Allocation)など，評価行列を次元削減することで「アクション好き」などの潜在的な意味を表現できるとされている手法。暗黙的データでも予測ができる。  
- 行列分解：もともとは未評価と不支持の差を考慮しなければならないので，明示的データでなければならなかったが，近年では暗黙的なデータへの拡張が行われている。  



#### 7.2.7  内容ベースフィルタリング

映画のタイトルや監督，ジャンルや俳優，口コミなど，アイテムを表現する情報に着目し，それらの過去のデータから推薦を行う。  
...
- ジャンルやテキストに含まれている単語が似ていなくても良いため，多様性のある推薦結果が得られる確率が高い。
- ドメイン知識を管理する必要がない  

協調フィルタリングの不得手

- 新規ユーザや新アイテムに対するデータが少なくなりがちで推薦が特に困難（コールドスタート問題）  
...
- 新しいサービスで行動データが蓄積されていなくても，比較適切な推薦がしやすい。  

内容ベースフィルタリングの不得手  

- 特に日本語だとデータを形態素解析するための辞書をメンテナンスする必要があるなど，ドメインに特化した情報をどのように扱うかが課題となる。  
...
- 正解率：例えば5段階評価で4以上の評価値が付与されていた場合に適合するとみなして，予測結果とユーザの評価値を比較する。  
- 適合率：予測結果に対する正解の割合  
- 再現率：真の正解に対するカバー率  

以下は主に回帰問題の評価に使われる尺度。  

- 平均絶対誤差：予測値と実測値の差の絶対値を評価用データで算出し，ズレを見る。  
- 平均二乗誤差：予測値と実測値の二乗した差をもとにしたデータ。平均絶対誤差と比べて外れ値に弱いとされている。  
- 順位相関：推薦するアイテムの並びを評価する指標。ランキング学習でよく利用される。  

これ以外にも「多様性」や「被覆率」を評価指標として使う場合もある。  



### 7.3  MovieLensのデータの傾向を見る

`TestScikitLearn\src\movielens\main.py`参照  
